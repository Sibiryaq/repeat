    ...
    private void load(HttpExchange h) throws IOException {
        try (h) { //try-with-resources
            ...
            if ("GET".equals(h.getRequestMethod())) {
                ...
                if (key.isEmpty()) {
                    System.out.println("Ключ для хранения пустой. key указывается в путе: /save/{key}");
                    h.sendResponseHeaders(400, 0);
                    return;
                }
                String value = readText(h);
                if (value.isEmpty()) {
                    System.out.println("Value для сохранения пустой, указывается в теле запроса");
                    h.sendResponseHeaders(400, 0);
                }
                String body = data.get(key);
                sendText(h, body);
            } else {
                System.out.println("/save ждем POST-запрос, а получил: " + h.getRequestMethod());
